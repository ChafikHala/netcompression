...
def _fro_to_name(fro_norm: float) -> str:
...
def _prepare_cfg(cfg, fro_norm: float):
    cfg = load_config_dict_like(cfg)

    _set_nested_attr(cfg, ["experiment", "name"], f"model_one_layer_FC_fro_{_fro_to_name(fro_norm)}")
...
def train_one_fro_norm(cfg, fro_norm: float, seed: Optional[int] = None) -> dict:
    cfg = load_config_dict_like(cfg)
    if seed is not None:
    ...
    target_fro_norm = float(fro_norm)
    ...
    for epoch in range(max_epochs):
        ...
        record = {
            "epoch": epoch + 1,
            "fro_norm_target": float(fro_norm),
...
def main() -> None:
    ...
    parser.add_argument("--fro-norm", type=float, required=True)
    parser.add_argument("--seed", type=int, required=False, default=None)
    args = parser.parse_args()

    cfg = load_config(args.config)
    train_one_fro_norm(cfg, fro_norm=float(args.fro_norm), seed=args.seed)
