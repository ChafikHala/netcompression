...
def plot_ratio_figure(agg: dict[str, dict], save_path: Path) -> None:
    ...
    robust_mean = np.array([agg[str(f)]["robust_over_clean_mean"] for f in fro_norms])
    robust_std = np.array([agg[str(f)]["robust_over_clean_std"] for f in fro_norms])

    uae_mean = np.array([agg[str(f)]["uae_over_clean_mean"] for f in fro_norms])
    uae_std = np.array([agg[str(f)]["uae_over_clean_std"] for f in fro_norms])
    ...
    for mean, std, marker, color, label in curves:
        ax.fill_between(fro_norms, mean - std, mean + std, color=color, alpha=0.12, linewidth=0)
        ax.errorbar(
            fro_norms,
            mean,
            yerr=std,
...
def main() -> None:
    ...
    if args.save_dir is None:
        args.save_dir = f"outputs/compressibility_adv_robustness/fro_norm_sweep_rob_and_uae_{args.norm}"
    ...
    plot_ratio_figure(agg, save_dir / "robust_and_uae_over_clean_vs_fro_norm.png")

    print(f"\nSaved raw results to: {save_dir / 'raw_results.json'}")
    print(f"Saved aggregated results to: {save_dir / 'aggregated_results.json'}")
    print(f"Saved figure to: {save_dir / 'robust_and_uae_over_clean_vs_fro_norm.png'}")
