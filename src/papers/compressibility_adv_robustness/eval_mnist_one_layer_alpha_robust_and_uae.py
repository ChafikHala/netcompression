...
#     u = torch.zeros(1, 1, 28, 28, device=device)
...
#             x_pert_norm = normalize_mnist_tensor(x_pert)
...
#         x_u_norm = normalize_mnist_tensor(x_u)
...
#     robust_mean = np.array([agg[str(a)]["robust_over_clean_mean"] for a in alphas])
#     robust_std = np.array([agg[str(a)]["robust_over_clean_std"] for a in alphas])

#     uae_mean = np.array([agg[str(a)]["uae_over_clean_mean"] for a in alphas])
#     uae_std = np.array([agg[str(a)]["uae_over_clean_std"] for a in alphas])
...
#         ax.fill_between(alphas, mean - std, mean + std, color=color, alpha=0.12, linewidth=0)
#         ax.errorbar(
#             alphas,
#             mean,
#             yerr=std,
...
#         args.save_dir = f"outputs/compressibility_adv_robustness/alpha_sweep_rob_and_uae_{args.norm}"
...
#     plot_ratio_figure(agg, save_dir / "robust_and_uae_over_clean_vs_alpha.png")

#     print(f"\nSaved raw results to: {save_dir / 'raw_results.json'}")
#     print(f"Saved aggregated results to: {save_dir / 'aggregated_results.json'}")
#     print(f"Saved figure to: {save_dir / 'robust_and_uae_over_clean_vs_alpha.png'}")
...
def _universal_ascent_step(grad: torch.Tensor, step_size: float, norm: str) -> torch.Tensor:
    norm = norm.lower()
...
def compute_pgd_based_uae(
    ...
    if norm == "linf":
        eps = float(eps_linf)
        if step_size_linf is None:
            step_size_linf = eps / 20.0
        step_size = float(step_size_linf)
        projector = _project_linf
    elif norm == "l2":
        eps = float(eps_l2)
        if step_size_l2 is None:
            step_size_l2 = eps / 20.0
        step_size = float(step_size_l2)
        projector = _project_l2
    else:
        raise ValueError("norm must be 'linf' or 'l2'")
    ...
    if random_start:
        if norm == "linf":
            u = torch.empty(1, 1, 28, 28, device=device).uniform_(-eps, eps)
        else:
            u = torch.randn(1, 1, 28, 28, device=device)
            u = _project_l2(u, eps=eps)
    else:
        u = torch.zeros(1, 1, 28, 28, device=device)
    ...
    for epoch in range(int(num_epochs)):
        for batch_idx, (x_raw, y) in enumerate(source_loader):
            ...
            if only_correct_clean:
                with torch.no_grad():
                    clean_logits = model(normalize_mnist_tensor(x_raw))
                    clean_pred = clean_logits.argmax(dim=1)
                    mask = clean_pred.eq(y)
            ...
            x_adv_raw = (x_raw + u).clamp(0.0, 1.0)
            x_adv_norm = normalize_mnist_tensor(x_adv_raw)
            logits_adv = model(x_adv_norm)

            loss = criterion(logits_adv, y)
            grad_u = torch.autograd.grad(loss, u, retain_graph=False, create_graph=False)[0]

            delta = _universal_ascent_step(grad_u, step_size=step_size, norm=norm)
            u = projector(u + delta, eps=eps).detach()
...
def main() -> None:
    ...
    parser.add_argument("--max-source-batches", type=int, default=None)
    parser.add_argument("--uae-random-start", action="store_true")
    parser.add_argument("--uae-only-correct-clean", action="store_true")
    ...
    for alpha in alphas:
        ...
        for seed in args.seeds:
            ...
            u = compute_pgd_based_uae(
                ...
                max_source_batches=args.max_source_batches,
                random_start=args.uae_random_start,
                only_correct_clean=args.uae_only_correct_clean,
            )
